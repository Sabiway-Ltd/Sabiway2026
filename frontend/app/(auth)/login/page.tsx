...
export default function Login() {
  ...
  const { login, reviewLogin, demoLogin, loading } = useAuthStore();
  ...
  return <main className="min-h-screen bg-[#f4f5f4] text-[#173126] lg:grid lg:grid-cols-[.9fr_1.1fr]">
    ...
    <section className="flex min-h-screen items-center justify-center px-4 py-10 sm:px-8"><div className="w-full max-w-md"><Link href="/" className="mx-auto mb-10 block w-fit lg:hidden"><Image src="/Footerlogo.svg" alt="SabiWay" width={140} height={44}/></Link><p className="text-sm text-[#66756d]">Welcome back</p><h2 className="mt-1 text-3xl font-black tracking-[-.025em]"><span className="text-[#008753]">Sign in</span> and continue your journey with SabiWay.</h2>
      ...
      <section className="mt-6 rounded-2xl border border-[#d7e8df] bg-white p-4" aria-label="Temporary demo access"><p className="text-xs font-black uppercase tracking-[.14em] text-[#008753]">Frontend preview</p><p className="mt-1 text-sm leading-6 text-[#5f6f67]">The backend is not connected yet. Use temporary demo access to review the internal product screens now.</p><button type="button" onClick={handleDemoLogin} className="mt-3 min-h-12 w-full rounded-xl bg-[#173126] px-4 text-sm font-black text-white transition hover:bg-[#24493a] focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#008753]/30">Enter Demo</button></section>

      {reviewModeEnabled ? <section className="mt-4 rounded-2xl border border-[#f1d57b] bg-[#fff8dc] p-4" aria-label="Internal review access"><p className="text-xs font-black uppercase tracking-[.14em] text-[#7a5a00]">Internal review mode</p><p className="mt-1 text-sm leading-6 text-[#5d511f]">Open the internal product without email, password or Google authentication. Review sessions are non-staff and available only when the backend is running in explicit development review mode.</p><div className="mt-3 grid grid-cols-2 gap-2"><button type="button" disabled={loading || reviewingRole !== null} onClick={() => void handleReviewLogin("client")} className="min-h-11 rounded-xl border border-[#d9bd57] bg-white px-3 text-sm font-black text-[#173126] disabled:opacity-50">{reviewingRole === "client" ? "Opening…" : "Review as Client"}</button><button type="button" disabled={loading || reviewingRole !== null} onClick={() => void handleReviewLogin("professional")} className="min-h-11 rounded-xl bg-[#008753] px-3 text-sm font-black text-white disabled:opacity-50">{reviewingRole === "professional" ? "Opening…" : "Review as Professional"}</button></div></section> : null}
