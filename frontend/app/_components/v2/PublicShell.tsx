...
export function PublicHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/90">
      ...
      {open ? <div id="mobile-primary-navigation" className="border-t border-border bg-card px-4 py-4 xl:hidden"><nav className="mx-auto grid max-w-7xl gap-1" aria-label="Mobile navigation">{navigation.map((item) => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="flex min-h-11 items-center rounded-xl px-3 py-3 font-bold text-foreground hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">{item.label}</Link>)}<Link href="/sabiforum" onClick={() => setOpen(false)} className="flex min-h-11 items-center rounded-xl px-3 py-3 font-bold text-foreground hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">SabiForum</Link><div className="mt-2 grid grid-cols-2 gap-2 border-t border-border pt-3 md:hidden"><Link href="/login" onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-center rounded-xl border border-border px-3 py-3 text-center font-extrabold">Sign in</Link><Link href="/signup" onClick={() => setOpen(false)} className="flex min-h-11 items-center justify-center rounded-xl bg-accent px-3 py-3 text-center font-black text-accent-foreground">Join</Link></div></nav></div> : null}
    </header>
  );
}
...
export function PublicShell({ children }: { children: React.ReactNode }) {
  return <div className="flex min-h-screen flex-col bg-background text-foreground"><SkipLink /><PublicHeader /><div id="main-content" tabIndex={-1} className="min-w-0 outline-none">{children}</div><PublicFooter /></div>;
}
