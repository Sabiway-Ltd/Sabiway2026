  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/90">
      <div className="mx-auto flex min-h-[72px] max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex min-h-11 shrink-0 items-center rounded-2xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2" aria-label="SabiWay home">
          <BrandLockup />
        </Link>

        <nav className="hidden items-center gap-0.5 xl:flex" aria-label="Primary navigation">
          {navigation.map((item) => (
            <Link key={item.href} href={item.href} className="flex min-h-11 items-center rounded-xl px-3 py-2 text-sm font-bold text-muted-foreground transition-colors hover:bg-muted hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="grid gap-10 border-b border-white/15 pb-10 lg:grid-cols-[1.35fr_2.65fr]">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-9 sm:grid-cols-4">
            {footerGroups.map((group) => (
              <div key={group.title}>
                <p className="text-xs font-black uppercase tracking-[.16em] text-white/70">{group.title}</p>
                <div className="mt-3 grid gap-1 text-sm font-semibold text-white/85">
                  {group.links.map((link) => <Link key={link.href} className="flex min-h-10 items-center rounded-lg hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" href={link.href}>{link.label}</Link>)}
                </div>
              </div>
            ))}
          </div>
...
export function V2ContentHero({ eyebrow, title, description }: { eyebrow: string; title: string; description: string }) {
  return (
    <section className="px-4 pt-8 sm:px-6 sm:pt-10 lg:px-8">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[var(--sabi-radius-xl)] bg-primary px-6 py-11 text-primary-foreground shadow-[var(--sabi-shadow-lg)] sm:px-10 sm:py-14 lg:px-14 lg:py-16">
        <p className="text-xs font-black uppercase tracking-[.18em] text-white/85">{eyebrow}</p>
        <h1 className="mt-3 max-w-4xl text-3xl font-black leading-tight tracking-[-.025em] sm:text-4xl lg:text-5xl">{title}</h1>
