...
export default function CommunityNavbar({ onCreatePost }: CommunityNavbarProps) {
  ...
  const { filterBySearch, resetFilteredResults } = usePostStore();
  ...
  const handleSearchKey = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && searchQuery.trim() !== "") {
      ...
      let type = "posts";
      if (trimmed.startsWith("@")) type = "profiles";
      else if (trimmed.startsWith("#")) type = "hashtags";

      const query = trimmed.replace(/^[@#]/, ""); // remove @ or # symbol
      filterBySearch(query, type);
        ...
        <div className="flex items-center gap-x-4">
          ...
          <button
            onClick={() => {
              ...
              resetFilteredResults(); // 🧹 Clear active hashtag/search
