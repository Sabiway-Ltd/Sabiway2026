...
import { AlertTriangle, ArrowRight, FileUp, MessageCircle, Search, ShieldAlert } from "lucide-react";

import { environment } from "@/app/config/environment";
import { useAuthStore } from "@/app/store/useAuthStore";

type Profile = { user_id: number; full_name: string; username: string; role?: string | null; job?: string | null };
type Thread = { id: string; client: Profile; professional: Profile; status: string; unread_count: number; booking_id?: string | null; is_blocked_by_me?: boolean; is_blocked_by_other?: boolean };
...
type Booking = { id: string; thread: string; client: Profile; professional: Profile; scope_summary: string; agreed_price?: string | null; currency: string; requested_for?: string | null; timezone: string; schedule_status: string; status: string };
...
function counterparty(thread: Thread, role?: string | null) { return role === "professional" ? thread.client : thread.professional; }
...
export default function MessagesClient() {
  ...
  const visibleThreads = useMemo(() => {
    ...
    return q ? threads.filter((thread) => {
      const other = counterparty(thread, user?.role);
      return `${other.full_name} ${other.job ?? ""}`.toLowerCase().includes(q);
    }) : threads;
  }, [query, threads, user?.role]);
  ...
  async function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    if (!activeId || messagingBlocked) return;
    setError("");
    setNotice("");
    ...
    if (!response.ok) {
      setError(payload.non_field_errors?.[0] || payload.detail || "Message could not be sent.");
      return;
    }
  ...
  async function createBooking(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`${environment.djangoUrl}/api/marketplace/bookings/`, {
      ...
      body: JSON.stringify({
        ...
        agreed_price: data.get("agreed_price") || null,
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      setError(payload.non_field_errors?.[0] || payload.detail || "Booking agreement could not be created.");
      return;
    }
    setBooking(payload as Booking);
    setNotice("Booking summary created. Continue service management in Bookings & schedules.");
    await loadThreads();
  }
  ...
  async function safetyAction(action: "block" | "unblock" | "report") {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      setError(payload.detail || payload.non_field_errors?.[0] || `Could not ${action} this conversation.`);
      return;
    }
    setNotice(action === "report" ? "Report sent to SabiWay support." : action === "unblock" ? "User unblocked." : "User blocked.");
  ...
  const providerPayout = amount - fee;
  ...
  return <main className="min-h-screen bg-[#f5f6f5] text-[#173126]">
    <div className="mx-auto grid max-w-7xl gap-3 px-3 py-4 sm:px-6 lg:grid-cols-[300px_minmax(0,1fr)_330px] lg:px-8">
      <aside className="overflow-hidden rounded-2xl border border-[#e0e6e2] bg-white">
        <div className="bg-[#008753] p-4 text-white">
          <div className="flex items-center justify-between"><h1 className="text-xl font-black">Messages</h1><span className="rounded-full bg-white/15 px-3 py-1 text-xs font-bold">{threads.length}</span></div>
          <label className="relative mt-4 block"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#66756d]" size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search" className="min-h-10 w-full rounded-xl border-0 bg-white pl-9 pr-3 text-sm text-[#173126] outline-none"/></label>
        </div>
        <div className="max-h-[72vh] overflow-y-auto">{visibleThreads.length === 0 ? <p className="p-5 text-sm text-[#68776f]">No conversations yet.</p> : visibleThreads.map((thread) => {
          const other = counterparty(thread, user?.role);
          return <button key={thread.id} onClick={() => setActiveId(thread.id)} className={`flex w-full gap-3 border-b border-[#edf0ee] p-3 text-left ${activeId === thread.id ? "bg-[#edf8f2]" : "hover:bg-[#fafbfa]"}`}><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#dff7eb] font-black text-[#008753]">{other.full_name.slice(0, 1)}</span><span className="min-w-0 flex-1"><span className="flex justify-between gap-2"><b className="truncate text-sm">{other.full_name}</b>{thread.unread_count > 0 && <span className="rounded-full bg-[#008753] px-2 py-0.5 text-[10px] font-black text-white">{thread.unread_count}</span>}</span><span className="mt-1 block truncate text-xs text-[#748078]">{user?.role === "professional" ? "Client" : other.job || "SabiWay professional"}</span></span></button>;
        })}</div>
      </aside>

      <section className="flex min-h-[72vh] flex-col overflow-hidden rounded-2xl border border-[#e0e6e2] bg-white">{active ? <>
        <header className="flex items-center justify-between border-b border-[#e9edeb] p-3"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#dff7eb] font-black text-[#008753]">{activeCounterparty?.full_name.slice(0, 1)}</span><div><p className="font-black">{activeCounterparty?.full_name}</p><p className="text-[11px] text-[#7c8781]">SabiWay conversation</p></div></div><div className="flex gap-2"><button onClick={() => void safetyAction("report")} className="rounded-full border border-[#dfe4e1] px-3 py-2 text-xs font-bold"><ShieldAlert size={13} className="mr-1 inline"/>Report</button><button onClick={() => void safetyAction(active.is_blocked_by_me ? "unblock" : "block")} className="rounded-full border border-[#dfe4e1] px-3 py-2 text-xs font-bold text-red-700">{active.is_blocked_by_me ? "Unblock" : "Block"}</button></div></header>
        {messagingBlocked && <div className="bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-900">Messaging is restricted in this conversation.</div>}
        <div className="flex-1 space-y-3 overflow-y-auto bg-[#f6f7f6] p-4">{messages.length === 0 ? <div className="grid h-full place-items-center text-center text-sm text-[#718078]"><div><MessageCircle className="mx-auto mb-2 text-[#008753]"/><p>Discuss the work, price and availability here.</p></div></div> : messages.map((message) => {
          const mine = message.sender.user_id === user?.id;
          return <article key={message.id} className={`w-fit max-w-[78%] rounded-2xl p-3 ${mine ? "ml-auto rounded-br-md bg-[#dff4e9]" : "rounded-bl-md bg-white shadow-sm"}`}>{!mine && <p className="text-[10px] font-black text-[#008753]">{message.sender.full_name}</p>}{message.body && <p className="mt-1 whitespace-pre-wrap text-sm leading-6">{message.body}</p>}{message.attachment && <a href={message.attachment} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[#008753]"><FileUp size={14}/>{message.attachment_name || "Attachment"}</a>}<p className="mt-1 text-right text-[9px] text-[#829087]">{new Date(message.created_at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p></article>;
        })}</div>
        <form onSubmit={sendMessage} className="border-t border-[#e9edeb] bg-white p-3"><div className="flex items-end gap-2"><label className={`flex h-10 w-10 cursor-pointer items-center justify-center rounded-full ${messagingBlocked ? "opacity-40" : "hover:bg-[#f1f4f2]"}`}><FileUp size={17}/><input disabled={messagingBlocked} name="attachment" type="file" accept="image/jpeg,image/png,image/webp,application/pdf,text/plain" className="hidden"/></label><textarea name="body" disabled={messagingBlocked} className={`${field} min-h-11 max-h-28 flex-1 rounded-2xl py-3`} placeholder={messagingBlocked ? "Messaging restricted" : "Leave a comment"}/><button disabled={messagingBlocked} className="min-h-11 rounded-full bg-[#008753] px-5 text-sm font-black text-white disabled:opacity-40">Send</button></div></form>
      </> : <div className="grid flex-1 place-items-center p-8 text-center text-[#718078]"><div><MessageCircle className="mx-auto mb-3 text-[#008753]"/><p className="font-bold">Choose a conversation</p></div></div>}</section>

      <aside className="rounded-2xl border border-[#e0e6e2] bg-white p-4">
        <p className="text-[10px] font-black uppercase tracking-[.15em] text-[#008753]">Work context</p>
        <h2 className="mt-1 text-lg font-black">Booking summary</h2>
        {!active ? <p className="mt-4 text-sm text-[#68776f]">Choose a conversation.</p> : booking ? <div className="mt-4 space-y-4">
          <div className="rounded-xl bg-[#f5f7f6] p-3"><p className="text-sm font-bold">{booking.scope_summary}</p><div className="mt-3 flex justify-between text-xs"><span className="text-[#748078]">Agreed price</span><b>{booking.currency} {money(amount)}</b></div><div className="mt-2 flex justify-between text-xs"><span className="text-[#748078]">Status</span><b className="capitalize text-[#008753]">{booking.status.replace("_", " ")}</b></div><div className="mt-2 flex justify-between text-xs"><span className="text-[#748078]">Schedule</span><b className="capitalize">{booking.schedule_status?.replace("_", " ") || "not set"}</b></div></div>
          {booking.currency === "NGN" && <div className="rounded-xl bg-[#fff7da] p-3 text-xs"><p className="font-black text-[#755900]">SabiPay estimate</p><div className="mt-2 flex justify-between"><span>10% platform fee</span><b>₦{money(fee)}</b></div><div className="mt-1 flex justify-between"><span>Provider receives</span><b>₦{money(providerPayout)}</b></div></div>}
          <div className="rounded-xl border border-[#dfe7e2] p-3"><p className="text-xs font-bold text-[#627269]">Acceptance, funded-work progression, schedule proposals and completion are managed in one canonical workspace.</p><Link href="/bookings" className="mt-3 inline-flex min-h-10 items-center gap-1 text-sm font-black text-[#008753]">Open Bookings & schedules <ArrowRight size={15}/></Link></div>
        </div> : user?.role === "client" ? <form onSubmit={createBooking} className="mt-4 space-y-3"><p className="text-sm text-[#68776f]">Create the booking summary here once scope and price are agreed. After creation, manage its lifecycle in Bookings & schedules.</p><textarea name="scope_summary" required className={`${field} min-h-24 py-2`} placeholder="Agreed scope"/><input name="agreed_price" required type="number" min="0.01" step="0.01" className={field} placeholder="Agreed price"/><select name="currency" defaultValue="NGN" className={field}><option>NGN</option><option>GBP</option><option>USD</option><option>EUR</option></select><button className="w-full rounded-xl bg-[#008753] px-4 py-3 text-sm font-black text-white">Create booking summary</button></form> : <p className="mt-4 text-sm text-[#68776f]">The Client creates the initial booking summary after agreement. Booking lifecycle management then happens in Bookings & schedules.</p>}
        {notice && <p className="mt-4 rounded-xl bg-[#eaf8f1] p-3 text-xs font-bold text-[#006b42]">{notice}</p>}
        {error && <div className="mt-4 rounded-xl bg-red-50 p-3 text-xs font-bold text-red-700"><p className="flex gap-2"><AlertTriangle size={15}/>{error}</p><button onClick={() => { setError(""); void loadThreads(); if (activeId) void loadConversation(activeId); }} className="mt-2 rounded-lg border border-red-200 px-3 py-1.5">Retry</button></div>}
      </aside>
    </div>
  </main>;
}
