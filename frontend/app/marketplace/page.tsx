...
async function getMarketplaceData(): Promise<{ listings: MarketplaceListing[]; jobs: MarketplaceJob[]; categories: MarketplaceCategory[] }> {
  try {
    ...
    return { listings: Array.isArray(listingPayload) ? listingPayload : listingPayload.results ?? [], jobs: Array.isArray(jobPayload) ? jobPayload : jobPayload.results ?? [], categories: Array.isArray(categoryPayload) ? categoryPayload : categoryPayload.results ?? [] };
  } catch {
...
export const metadata = {
  title: "SabiWay Marketplace | Find trusted services by location",
  description: "Find trusted professionals near where a service is needed, search another location, or discover remote services across markets.",
};

export default async function MarketplacePage() {
  const { listings, jobs, categories } = await getMarketplaceData();
  return <MarketplaceShell><MarketplaceClient initialListings={listings} initialJobs={jobs} categories={categories} /></MarketplaceShell>;
}
