...
export default function MarketplaceClient({ initialListings, initialJobs, categories }: Props) {
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings.filter((item) => {
      const searchMatch = !needle || [item.title, item.description, item.category.name, item.subcategory?.name ?? "", item.provider.full_name, item.provider.job ?? ""].some((value) => value.toLowerCase().includes(needle));
      const locationMatch = !place || [item.area, item.city ?? "", item.state, item.country ?? ""].some((value) => value.toLowerCase().includes(place));
      return searchMatch && locationMatch && (!categoryFilter || item.category.slug === categoryFilter);
    });
  ...
  return (
    <main className="min-h-screen bg-[#f7faf8] text-[#173126]">
      <header className="border-b border-[#0a6f48] bg-[#008753] text-white"><div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8"><Link href="/" className="text-2xl font-black">SabiWay</Link><nav className="flex items-center gap-2 text-sm font-semibold sm:gap-4"><Link href="/marketplace" className="rounded-lg bg-white/15 px-3 py-2">Marketplace</Link><Link href="/messages" className="px-3 py-2">Messages</Link><Link href="/community" className="px-3 py-2">SabiForum</Link><Link href="/login" className="rounded-lg bg-[#FFB800] px-4 py-2 text-[#173126]">Sign in</Link></nav></div></header>

      <section className="bg-[#008753] px-4 pb-12 pt-10 text-white sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><div className="max-w-3xl"><p className="mb-3 inline-flex items-center gap-2 rounded-full bg-white/12 px-3 py-1 text-sm font-semibold"><Sparkles size={16}/> Nigerians helping Nigerians</p><h1 className="text-4xl font-black leading-tight sm:text-5xl">Find trusted Nigerian professionals near you or anywhere.</h1><p className="mt-4 max-w-2xl text-base leading-7 text-white/85">Discover services, post a job, message safely, agree scope and price, then schedule the work in one auditable journey.</p></div><div className="mt-8 grid gap-3 rounded-2xl bg-white p-3 shadow-xl md:grid-cols-[1fr_1fr_220px_auto]"><label className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#708077]" size={18}/><input value={query} onChange={(e) => setQuery(e.target.value)} className={`${inputClass} pl-11`} placeholder="What service or problem do you have?"/></label><label className="relative"><MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-[#708077]" size={18}/><input value={location} onChange={(e) => setLocation(e.target.value)} className={`${inputClass} pl-11`} placeholder="City, state or country"/></label><select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className={inputClass}><option value="">All categories</option>{categories.map((c) => <option key={c.id} value={c.slug}>{c.name}</option>)}</select><button className="min-h-12 rounded-xl bg-[#FFB800] px-5 font-extrabold text-[#173126]">Search</button></div></div></section>

      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="mb-10"><div className="mb-4 flex items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[.14em] text-[#008753]">Browse services</p><h2 className="mt-1 text-2xl font-black">Popular categories</h2></div><button onClick={() => setCategoryFilter("")} className="text-sm font-bold text-[#008753]">View all</button></div><div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">{categories.slice(0,10).map((category) => <button key={category.id} onClick={() => { setTab("services"); setCategoryFilter(category.slug); }} className="rounded-2xl border border-[#dce7e1] bg-white p-4 text-left shadow-sm hover:border-[#008753]"><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-[#e8f7f0] text-[#008753]"><BriefcaseBusiness size={20}/></div><p className="font-extrabold">{category.name}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-[#6c7a72]">{category.description}</p></button>)}</div></section>

        <section><div className="flex flex-col gap-4 border-b border-[#dce7e1] pb-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex rounded-xl bg-[#eaf4ef] p-1"><button onClick={() => setTab("services")} className={`rounded-lg px-4 py-2 text-sm font-extrabold ${tab === "services" ? "bg-white text-[#008753] shadow-sm" : "text-[#53645a]"}`}>Find services</button><button onClick={() => setTab("jobs")} className={`rounded-lg px-4 py-2 text-sm font-extrabold ${tab === "jobs" ? "bg-white text-[#008753] shadow-sm" : "text-[#53645a]"}`}>Open jobs</button></div><div className="flex flex-wrap gap-2"><button onClick={() => { setShowPostJob((v) => !v); setShowOfferService(false); }} className="rounded-xl border border-[#008753] px-4 py-2 text-sm font-extrabold text-[#008753]">Post a job</button><button onClick={() => { setShowOfferService((v) => !v); setShowPostJob(false); }} className="rounded-xl bg-[#008753] px-4 py-2 text-sm font-extrabold text-white">Offer a service</button></div></div>

          {(showPostJob || showOfferService) && <div className="my-6 rounded-2xl border border-[#dce7e1] bg-white p-5 shadow-sm"><h3 className="text-xl font-black">{showPostJob ? "Tell professionals what you need" : "Create your service listing"}</h3><form onSubmit={showPostJob ? postJob : postService} className="mt-5 grid gap-3 md:grid-cols-2"><select name="category_id" required className={inputClass}><option value="">Choose category</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select><input name="title" required className={inputClass} placeholder={showPostJob ? "What do you need done?" : "Service title"}/><textarea name="description" required className={`${inputClass} min-h-28 py-3 md:col-span-2`} placeholder="Describe the work clearly"/>{showPostJob ? <><input name="budget_min" type="number" min="0" className={inputClass} placeholder="Minimum budget (NGN)"/><input name="budget_max" type="number" min="0" className={inputClass} placeholder="Maximum budget (NGN)"/><input name="needed_by" type="date" className={inputClass}/></> : <><input name="price_from" required type="number" min="0" className={inputClass} placeholder="Starting price (NGN)"/><input name="pricing_note" className={inputClass} placeholder="Pricing note"/><input name="availability_text" className={inputClass} placeholder="Availability"/></>}<select name="delivery_mode" defaultValue="in_person" className={inputClass}><option value="in_person">In person</option><option value="remote">Remote</option><option value="both">In person or remote</option></select><input name="country" className={inputClass} placeholder="Country"/><input name="state" className={inputClass} placeholder="State/region"/><input name="city" className={inputClass} placeholder="City"/><input name="area" className={inputClass} placeholder="Area"/>{!showPostJob && <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" name="available_now"/> Available now</label>}<div className="md:col-span-2 flex gap-3"><button className="rounded-xl bg-[#008753] px-5 py-3 font-extrabold text-white">Submit for review</button><button type="button" onClick={() => { setShowPostJob(false); setShowOfferService(false); }} className="rounded-xl border border-[#d6e0da] px-5 py-3 font-bold">Cancel</button></div></form></div>}

          {notice && <div className="my-4 flex items-center gap-2 rounded-xl bg-[#eaf8f1] p-4 text-sm font-bold text-[#006b42]"><CheckCircle2 size={18}/>{notice}</div>}{error && <div className="my-4 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}
