...
def create_or_sync_profile_for_user(sender, instance, created, **kwargs):
    """Create the profile once and keep its legacy role mirror aligned to User.role.

    User.role is the authoritative role. Profile.role remains only as a transitional
    compatibility field for older code paths and must never be independently edited.
    """
    if created:
...
def ensure_profile_derived_fields(sender, instance, **kwargs):
    ...
    if instance.user_id:
        # Transitional compatibility only: account User.role remains authoritative.
