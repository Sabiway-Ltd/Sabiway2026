...
def paystack_request(method, path, *, payload=None, params=None):
    ...
    try:
        ...
        try:
            record_technical_metric("payment_provider", route=route, success=False, metadata={"error_code": "network_or_invalid_response"})
        except Exception:
            pass
    ...
    try:
        record_technical_metric("payment_provider", route=route, status_code=response.status_code, success=success)
    except Exception:
        pass
    if not success:
...
def initialize_payment(*, email, amount_subunit, currency="NGN", reference, callback_url, metadata):
    return paystack_request("POST", "/transaction/initialize", payload={"email": email, "amount": str(amount_subunit), "currency": currency.upper(), "reference": reference, "callback_url": callback_url, "metadata": metadata})
...
def list_banks(country="nigeria", currency="NGN"):
    return paystack_request("GET", "/bank", params={"country": country, "currency": currency.upper(), "perPage": 100})
...
def create_transfer_recipient(*, name, account_number, bank_code, metadata, currency="NGN"):
    return paystack_request("POST", "/transferrecipient", payload={"type": "nuban", "name": name, "account_number": account_number, "bank_code": bank_code, "currency": currency.upper(), "metadata": metadata})
...
def initiate_refund(*, transaction_reference, amount_subunit, reason, currency="NGN"):
    return paystack_request("POST", "/refund", payload={"transaction": transaction_reference, "amount": amount_subunit, "currency": currency.upper(), "customer_note": reason[:240], "merchant_note": reason[:240]})
