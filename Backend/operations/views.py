...
class ProductEventIngestView(APIView):
    ...
    def post(self, request):
        ...
        event = record_product_event(event_name, actor=request.user, source=source, properties=request.data.get("properties") or {}, anonymous_id=request.data.get("anonymous_id") or "")
        if event is None:
...
class MeasurementSnapshotView(APIView):
    ...
    def get(self, request):
        if not (request.user.is_superuser or request.user.has_perm("operations.view_product_measurement")):
...
class SupportCaseViewSet(mixins.ListModelMixin,mixins.CreateModelMixin,mixins.RetrieveModelMixin,mixins.UpdateModelMixin,viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_staff and self.request.user.has_perm("operations.manage_support"):
            status_filter=self.request.query_params.get("status","").strip(); priority_filter=self.request.query_params.get("priority","").strip()
    ...
    def get_serializer_class(self):
        return SupportCaseStaffUpdateSerializer if self.action in {"update","partial_update"} else SupportCaseSerializer

    def perform_create(self, serializer):
        case=serializer.save(opened_by=self.request.user)
        record_operations_audit(actor=self.request.user,action="support_case_opened",target_type="support_case",target_id=case.id,new_state={"status":case.status,"category":case.category,"priority":case.priority})
    ...
    def perform_update(self, serializer):
        ...
        case=self.get_object(); previous={"status":case.status,"priority":case.priority,"assigned_to":case.assigned_to_id}; updated=serializer.save()
        record_operations_audit(actor=self.request.user,action="support_case_updated",target_type="support_case",target_id=updated.id,previous_state=previous,new_state={"status":updated.status,"priority":updated.priority,"assigned_to":updated.assigned_to_id})
        if previous["status"] != updated.status: notify_support_case(updated,actor=self.request.user)
