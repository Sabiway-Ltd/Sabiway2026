...
class SupportCaseAdmin(admin.ModelAdmin):
    ...
    fields = (
        "id", "opened_by", "category", "subject", "description", "status", "priority", "assigned_to",
        "reference_type", "reference_id", "internal_note", "resolved_at", "created_at", "updated_at",
    )

    def has_add_permission(self, request):
        return bool(request.user.is_superuser or request.user.has_perm("operations.manage_support"))
    ...
    def save_model(self, request, obj, form, change):
        ...
        if change and obj.pk:
            old = SupportCase.objects.get(pk=obj.pk)
            previous = {"status": old.status, "priority": old.priority, "assigned_to": old.assigned_to_id}
        if obj.status in {SupportCase.Status.RESOLVED, SupportCase.Status.CLOSED} and not obj.resolved_at:
            obj.resolved_at = timezone.now()
        elif obj.status not in {SupportCase.Status.RESOLVED, SupportCase.Status.CLOSED}:
            obj.resolved_at = None
        if not obj.opened_by_id and not change:
            obj.opened_by = request.user
        super().save_model(request, obj, form, change)
        record_operations_audit(
            actor=request.user,
            action="support_case_admin_updated" if change else "support_case_admin_created",
...
class PlatformConfigurationAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        return bool(request.user.is_superuser or request.user.has_perm("operations.manage_platform_config"))
    ...
    def save_model(self, request, obj, form, change):
        previous = {}
        if change and obj.pk:
            old = PlatformConfiguration.objects.get(pk=obj.pk)
            previous = {"value": old.value, "description": old.description}
        obj.full_clean()
        obj.updated_by = request.user
        super().save_model(request, obj, form, change)
        record_operations_audit(
            actor=request.user,
            action="platform_configuration_updated" if change else "platform_configuration_created",
            target_type="platform_configuration",
            target_id=obj.pk,
            previous_state=previous,
            new_state={"value": obj.value, "description": obj.description},
            metadata={"key": obj.key, "source": "django_admin"},
        )


@admin.register(OperationsAudit)
class OperationsAuditAdmin(admin.ModelAdmin):
