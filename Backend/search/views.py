...
class SearchView(APIView):
    """Bounded public discovery for SabiForum content, profiles and hashtags."""
    ...
    def get(self, request):
        ...
        if len(query) > self.maximum_query_length:
            return Response({"detail": f"Search cannot exceed {self.maximum_query_length} characters."}, status=status.HTTP_400_BAD_REQUEST)

        if search_type == "posts":
            posts = Post.objects.filter(content__icontains=query).order_by("-created_at")[: self.result_limit]
            return Response(PostListSerializer(posts, many=True, context={"request": request}).data)

        if search_type == "profiles":
            profiles = Profile.objects.select_related("user").filter(Q(username__icontains=query) | Q(full_name__icontains=query)).order_by("username")[: self.result_limit]
            return Response([
                {
        ...
        hashtags = Hashtag.objects.filter(tag__icontains=query).order_by("-use_count", "tag")[: self.result_limit]
