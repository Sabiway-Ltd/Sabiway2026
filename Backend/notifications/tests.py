...
@override_settings(EXPRESS_URL="http://realtime.test", NOTIFICATION_PUSH_DELIVERY_ENABLED=False, NOTIFICATION_EMAIL_DELIVERY_ENABLED=False)
class ForumNotificationSignalTests(TestCase):
    def make_profile(self, email, full_name, username):
        user = get_user_model().objects.create_user(email=email, full_name=full_name, password="StrongPassword123!")
        profile, _ = Profile.objects.get_or_create(user=user, defaults={"full_name": full_name, "username": username})
        if profile.username != username:
            profile.username = username; profile.full_name = full_name
    ...
    def test_like_notifies_post_author_and_uses_internal_header(self, mock_post):
        ...
        self.assertIn("x-sabiway-internal-token", mock_post.call_args.kwargs["headers"])
    ...
    def test_self_like_does_not_create_notification(self, mock_post):
        ...
        Notification.objects.all().delete(); mock_post.reset_mock()
    ...
    def test_reply_deduplicates_same_recipient(self, mock_post):
        ...
        Notification.objects.all().delete(); mock_post.reset_mock()
        Reply.objects.create(user=replier, comment=comment, parent_reply=parent, content="Nested reply")
        self.assertEqual(Notification.objects.filter(type="reply", user=author, actor=replier).count(), 1)
