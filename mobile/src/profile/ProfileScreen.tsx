...
export function ProfileScreen({ session, onOpenVerification }: Props) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setProfile(next);
      setForm((current) => ({ ...current, full_name: next.full_name, username: next.username }));
...
const styles = StyleSheet.create({
  ...
  role: { color: colors.primaryStrong, fontWeight: "700", marginTop: 2 },
  ...
  primaryButton: { minHeight: 48, borderRadius: 12, backgroundColor: colors.primary, justifyContent: "center", alignItems: "center", paddingHorizontal: 16 },
  primaryButtonText: { color: colors.onPrimary, fontWeight: "800" },
  secondaryButton: { minHeight: 44, borderRadius: 10, borderWidth: 1, borderColor: colors.primary, justifyContent: "center", paddingHorizontal: 12 },
  secondaryButtonText: { color: colors.primaryStrong, fontWeight: "800" },
  disabled: { opacity: 0.5 },
  error: { color: colors.danger, fontWeight: "600" },
  success: { color: colors.success, fontWeight: "700" },
  privacyNote: { color: colors.textMuted, fontSize: 12, lineHeight: 18, marginTop: 4 },
});
